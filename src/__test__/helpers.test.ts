...
describe("Test helper functions", () => {
  test("should detect incorrect characters", () => {
    expect(characterIsLetterWeHaveToCollect("He")).toBeFalsy();
  ...
  test("should detect correct characters", () => {
    ...
    expect(characterIsLetterWeHaveToCollect("Z")).toBeTruthy();
  });
  ...
  test("should throw error when tuple is not in range", () => {
    ...
    expect(() => calculateTheMoveBasedOnIndexes([Infinity, Infinity])).toThrowError(
      "Tuple containing the move is wrong"
    );
  });
});
